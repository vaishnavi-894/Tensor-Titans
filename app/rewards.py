...
def compute_reward(task: Dict[str, Any], state: EnvState, action: Action) -> Dict[str, Any]:
    ...
    if action.action_type == "inspect_policy":
        ...
        delta = 0.20 if action.value == gold["tag"] else 0.01
        reward += delta
        components["tag_issue"] = delta

    elif action.action_type == "set_priority":
        delta = 0.15 if action.value == gold["priority"] else 0.01
        reward += delta
        components["set_priority"] = delta

    elif action.action_type == "assign_team":
        delta = 0.20 if action.value == gold["team"] else 0.01
        reward += delta
        ...
        delta = max(0.01, min(0.15, hits * 0.04))
        reward += delta
    ...
    if state.step_count > 6:
        reward -= 0.02
        components["long_episode_penalty"] = 0.01

    # Clamp reward to strict (0, 1) open interval for grader compliance
    reward = _clamp(reward)

    # Clamp all component values to (0, 1)
    components = {k: _clamp(v) for k, v in components.items()}
